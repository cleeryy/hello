...
if [ -z "${API_TOKEN:-}" ]; then
  echo "API_TOKEN is required (server refuses to boot without it)"
  exit 1
fi
...
for _ in $(seq 1 30); do
  if curl -sf "${AUTH[@]}" "$API/health" > /dev/null; then
    break
  fi
  sleep 1
done
curl -sf "${AUTH[@]}" "$API/health" | jq .

echo "2. Add device1 (no IP, no ping)"
curl -sf "${AUTH[@]}" -X POST "$API/devices" \
...
echo "4. List devices"
curl -sf "${AUTH[@]}" "$API/devices" | jq .

echo "5. Wait for the monitor to ping"
sleep 3

echo "6. device2 should be up"
curl -sf "${AUTH[@]}" "$API/devices/device2" | jq .
...
if [ "${SKIP_WAKE:-0}" = "1" ]; then
  ...
  curl -sf "${AUTH[@]}" -X POST "$API/wake/AA:BB:CC:DD:EE:FF" | jq .
fi
...
if [ "${SKIP_WAKE:-0}" = "1" ]; then
  ...
  curl -sf "${AUTH[@]}" -X POST "$API/devices/device2/wake" | jq .
fi

echo "9. Delete device1 (204, empty body)"
code=$(curl -s "${AUTH[@]}" -o /dev/null -w "%{http_code}" -X DELETE "$API/devices/device1")
if [ "$code" != "204" ]; then
...
curl -sf "${AUTH[@]}" "$API/schedules" | jq .

echo "14. Pause then delete the schedule"
curl -sf "${AUTH[@]}" -X PUT "$API/schedules/e2e-morning" \
  -H "Content-Type: application/json" \
  -d '{"id":"e2e-morning","device_id":"device2","cron":"@daily","enabled":false}' | jq .
code=$(curl -s "${AUTH[@]}" -o /dev/null -w "%{http_code}" -X DELETE "$API/schedules/e2e-morning")
if [ "$code" != "204" ]; then
  echo "expected 204, got $code"
  exit 1
fi
...
curl -sf "${AUTH[@]}" -X POST "$API/discover" | jq .

echo "16. Reject an empty adopt batch (422)"
code=$(curl -s "${AUTH[@]}" -o /dev/null -w "%{http_code}" -X POST "$API/discover/adopt" \
  -H "Content-Type: application/json" \
  -d '{"hosts":[]}')
if [ "$code" != "422" ]; then
  echo "expected 422, got $code"
